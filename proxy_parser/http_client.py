...
class HTTPClient:
            ...
            async with SEMAPHORE:
                ...
                async with httpx.AsyncClient(
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"✓ GET {url} - Status: {response.status_code}, Size: {content_length} chars, Time: {elapsed_time:.2f}s")
                        return response.text
                    else:
                        logger.warning(f"✗ GET {url} - Status: {response.status_code}, Time: {elapsed_time:.2f}s")
                        return None
    ...
    async def get_json(self, url: str, proxy: Optional[str] = None, timeout: int = 10) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            async with SEMAPHORE:
                ...
                # httpx uses a different proxy format
                transport = None
                if proxy:
                ...
                async with httpx.AsyncClient(
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            logger.info(f"✓ JSON GET {url}{proxy_info} - Status: {response.status_code}, Time: {elapsed_time:.2f}s")
                            return json_data
                        except Exception as e:
                            logger.error(f"✗ JSON GET {url}{proxy_info} - Invalid JSON: {e}, Time: {elapsed_time:.2f}s")
                            return None
                    else:
                        logger.warning(f"✗ JSON GET {url}{proxy_info} - Status: {response.status_code}, Time: {elapsed_time:.2f}s")
                        return None
        except httpx.TimeoutException:
            elapsed_time = asyncio.get_event_loop().time() - start_time
            logger.warning(f"✗ JSON GET {url}{proxy_info} - Timeout after {elapsed_time:.2f}s")
            return None
        except Exception as e:
            elapsed_time = asyncio.get_event_loop().time() - start_time
            logger.error(f"✗ JSON GET {url}{proxy_info} - Error: {e}, Time: {elapsed_time:.2f}s")
            return None
...
class GitHubClient:
    ...
    def search_files(self, query: str, page: int = 1) -> Optional[List[str]]:
        ...
        try:
            ...
            if not response.ok:
                logger.warning(f"✗ GitHub search '{query}' - Status: {response.status_code}, Time: {elapsed_time:.2f}s")
                return None
            ...
            logger.info(f"✓ GitHub search '{query}' - Found {len(results)} results, Status: {response.status_code}, Time: {elapsed_time:.2f}s")
            return results
            
        except Exception as e:
            elapsed_time = time.time() - start_time
            logger.error(f"✗ GitHub search '{query}' - Error: {e}, Time: {elapsed_time:.2f}s")
            return None
