...
class ProxyOrchestrator:
    ...
    async def run_full_cycle(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ Step 2 completed - {len(unchecked_proxies)} proxies found in {step2_time:.2f}s"
            )

            # Save unchecked proxies
            logger.info("💾 Saving unchecked proxies")
            await self.parser.save_unchecked_proxies(
                unchecked_proxies, str(NOT_CHECKED_PROXIES_FILE)
            )
    ...
    async def check_proxies(self) -> None:
        ...
        success_rate = (
            (working_count / len(unchecked_proxies)) * 100 if unchecked_proxies else 0
        )

        logger.info(
            f"📊 Proxy validation summary: {working_count}/{len(unchecked_proxies)} working ({success_rate:.1f}% success rate) in {elapsed_time:.2f}s"
        )
    ...
    async def run_infinite_cycle(self, timeout_seconds: int) -> None:
        ...
        """
        logger.info(
            f"🔄 Starting infinite proxy parsing with {timeout_seconds}s timeout between cycles"
        )
        cycle_count = 0
